...
from tomo_center.ai.model_archs import _make_dinov2_model

log = tca_logging.getLogger(__name__)

def load_images(img_cache_original,downsample_factors,use_8bits,preprocessed):
    imgs_cache = []
...
def bin_inference_pipeline(args, img_cache_original, center_of_rotation_cache, out_dir, preprocessed=False):
    from tomo_center.ai.model_archs import RangeClassificationModel
    use_8bits = args.bin_infer_use_8bits
    downsample_factors = args.bin_infer_downsample_factor
    nums_windows = args.bin_infer_num_windows
    szs = args.bin_infer_window_size
    assert isinstance(downsample_factors,list)
    assert isinstance(nums_windows,list)
    assert isinstance(szs,list)
    num_frames = args.bin_infer_num_frames
    bin_size = args.bin_infer_bin_size
    aggregator_depth = args.bin_infer_aggregator_depth
    aggregator_num_heads = args.bin_infer_aggregator_num_heads
    seed_number = args.bin_infer_seed_number
    model_path = args.bin_infer_model_path
    if model_path == 'none':
        raise ValueError("--bin-infer-model-path must be set when using --rotation-axis-method ai.")
    ...
    if num_frames>1:
        ...
        multi_frames = False
    assert num_frames==2, f"Error: bin inference requires 2 frames. num_frames is currently set to {num_frames}."
    ...
    for idx in range(imgs_cache[0].shape[0]):
        samples = []
        cor = center_of_rotation_cache[idx]
        if (cor + bin_size) <= np.array(center_of_rotation_cache).max():
            diff = abs(np.array(center_of_rotation_cache)-(cor+bin_size))
            idx2 = diff.argmin()
        else:
            break
        ...
        for img_cache,patch_corners,sz in zip(imgs_cache,patches_corners,szs):
            ...
            for patch_corner in patch_corners:
                ...
                imgs.append(img)
            sample = {'images':torch.cat((sample['images'].unsqueeze(2),torch.cat(imgs,dim=1).unsqueeze(2)),dim=2)}
    ...
    t_stop3 = time.time()
    log.info(f"done. Elapsed time is {t_stop3-t_start3} s.")
    ...
    if args.bin_infer_save_intermediate_data:
        print(f"range is {center_of_rotation_cache[bin_idx]} and {center_of_rotation_cache[bin_idx2]}")
        np.savez(Path(out_dir)/'bin_scores_all',features_all,scores_all,center_of_rotation_cache)
...
def inference_pipeline(args, img_cache_original, center_of_rotation_cache, out_dir, preprocessed=False):
    from tomo_center.ai.model_archs import ClassificationModel
    ...
    imgs_cache = load_images(img_cache_original,downsample_factors,use_8bits,preprocessed)
    ...
    if args.infer_save_intermediate_data:
        np.savez(Path(out_dir)/'scores_all',features_all,scores,center_of_rotation_cache)
